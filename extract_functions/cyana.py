...
def export_cyana_prot(shifts_dict, output_dir="results/cyana_export"):
    ...
    for name, df in shifts_dict.items():
        ...
        is_math_string = df["sequence_code"].astype(str).str.contains("-", na=False) & ~df[
            "sequence_code"
        ].astype(str).str.startswith("-", na=False)
...
def generate_xeasy_header(spectrum_type, dimensions):
    ...
    labels = [
        str(dim).upper().replace("15N", "N").replace("13C", "C").replace("1H", "H")
    ...
    header = (
        f"# Number of dimensions {num_dimensions}\n"
...
def export_cyana_peaks(peak_dict, out_path="results/cyana_export"):
    ...
    for name, df in peak_dict.items():
        ...
        for idx, row in df.iterrows():
            ...
            vol_str = (
                f"{float(vol):.9e}".ljust(21) if pd.notna(vol) else "0.000000000e+00".ljust(21)
            )

            pos_str = "".join(
                f"{float(row.get(f'position_{i}', 0)):.4f}".ljust(11) for i in dim_indices
            )
