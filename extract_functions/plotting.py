...
def get_nmr_plot_config(ylim_csp=None, ylim_ratio=1.1, CSP=True, Int=True, Vol=True,
                        color_csp='tab:blue', color_int='tab:orange', color_vol='tab:red',
                        int_prefix='Norm_'):
    """
...
def plot_nmr_metrics(analysis_df, cutoff=0.01, ylim_csp=None, ylim_ratio=1.1,
                     save_path="results/plots", CSP=True, Int=True, Vol=True,
                     color_csp='tab:blue', color_int='tab:orange', color_vol='tab:red'):
    """
    ...
    config = get_nmr_plot_config(ylim_csp, ylim_ratio, CSP, Int, Vol,
                                 color_csp, color_int, color_vol, int_prefix)
    ...
    for category, settings in config.items():
        ...
        for spec_name in spec_list:
            ...
            res_code_str = df_plot['Res_Code'].fillna(0).astype(
            ...
            df_plot['Plot_X'] = np.where(
                is_unassigned,
                res_code_str,
                res_code_str + " " + df_plot['Res_Letter'].astype(str)
            )
            ...
            df_plot.plot.bar(
                ...
                width=0.8, edgecolor="black", linewidth=0.35,
                color=settings['color'], legend=False
            )
            ...
            if save_path:
                ...
                plt.savefig(current_save_path / f"{settings['prefix']}_{spec_name}.png",
...
def plot_combined(analysis_df, ylim_csp=None, ylim_ratio=1.05, save_path="results/plots",
                  Int=True, Vol=False,
                  color_csp='tab:blue', color_int='tab:orange', color_vol='tab:red'):
    """
    ...
    config = get_nmr_plot_config(ylim_csp, ylim_ratio, CSP=True, Int=Int, Vol=Vol,
                                 color_csp=color_csp, color_int=color_int, color_vol=color_vol,
                                 int_prefix=int_prefix)
    csp_config = config['CSPs']

    secondary_metrics = {k: v for k, v in config.items(
    ) if k != 'CSPs' and v['active'] and k in levels}
    ...
    for metric_col, metric_settings in secondary_metrics.items():
        for spec_name in spec_list:
            ...
            ax1.set_xticks(ind)
            ax1.set_xticklabels(df_plot['Plot_X'], rotation=90, ha='center',
            ...
            plt.savefig(save_path / f"Combined_{metric_settings['prefix']}_{spec_name}.png",
                        bbox_inches='tight', dpi=300)
            plt.show()
            plt.close()
